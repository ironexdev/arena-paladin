...
return [
    ...
    CorsMiddleware::class => DI\factory(function (LoggerInterface $logger) {
        $allowedOrigins = [$_ENV["CLIENT_URL"]];

        return new CorsMiddleware([
            "origin" => $allowedOrigins,
            "methods" => RequestMethodEnum::toArray(),
            "headers.allow" => explode(",", $_ENV["ACCESS_CONTROL_ALLOW_HEADERS"]),
            "headers.expose" => explode(",", $_ENV["ACCESS_CONTROL_EXPOSE_HEADERS"]),
            "credentials" => $_ENV["ACCESS_CONTROL_ALLOW_CREDENTIALS"] === "true",
            "cache" => 0,
            "logger" => $logger
        ]);
    }),
